...
function updateProduct(index, userId, action){
    const shoppingCartUrl = "https://73ef-213-10-175-251.ngrok-free.app/api/shoppingcart?user_id=" + userId;
    ...
    .then(data => {
        return fetch("https://73ef-213-10-175-251.ngrok-free.app/api/products?product_id=" + data.product_ids[index])
        .then(response=>response.json())
        .then(product=>{
...
function updateTotal(userId){
    ...
    .then(data => {
            ...
            for (let i = 0; i < data.product_ids.length; i++) {
                fetch("https://73ef-213-10-175-251.ngrok-free.app/api/products?product_id=" + data.product_ids[i])
                .then(response=>response.json())
                .then(product=>{
...
function addAmount(index, userId, productId){
    const apiUrl = 'https://73ef-213-10-175-251.ngrok-free.app/api/shoppingcart?user_id=' + userId + '&product_id=' + productId + '&action=add';
    fetch(apiUrl, {
        method: "PATCH",
        headers: {
...
function removeAmount(index, userId, productId){
    const apiUrl = 'https://73ef-213-10-175-251.ngrok-free.app/api/shoppingcart?user_id=' + userId + '&product_id=' + productId + '&action=delete';
