...
<body>
    ...
    <main class="container-fluid">
                ...
                ?>
                    <li id="dance-locations-address-light" class="rounded my-3 list-group-item text-center fw-semibold">
                        ...
                        <?php echo $danceLocation->getDanceLocationName() . " " . 
                            $danceLocation->getDanceLocationNumber() . ", ".  $danceLocation->getDanceLocationPostcode() . ", " . $danceLocation->getDanceLocationCity()?>
                    </li>
                    <?php }
                ?>
                </ul>
        ...
        <?php 
        // Check if the extraNote is not null, and print it
        if ($events[0]->getDanceEventExtraNote() != null) {
            echo '<p class="text-left text-light mb-4">'. $events[0]->getDanceEventExtraNote() . "</p>";
        }       
    ?>
        <?php } ?>
        ...
        <div class="row">
            <?php 
                foreach ($danceFlashbacks as $flashback){           
                ?>
            <div class="col-md-3 my-4">
                <img src="<?php echo $flashback->getDanceFlashbackUrl()?>"
                    alt="<?php echo $flashback->getDanceFlashbackCredit()?>'s Photo" class="card-img-top h-100 border">
                <p class="text-left text-light"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
                    ...
                    </svg><?php echo " ". $flashback->getDanceFlashbackCredit()?></p>
            </div>
            <?php }
                ?>
        </div>
